...
class XC7BSHandler:
    ...
    def setup(
        self,
        part_json_content: str,
        evo_bits: Dict[XC7BitPosition, bool],
        habitat_bits: List[XC7BitPosition] = None,
    ) -> None:
        """
        ...
        self.evo_frames = [
            frame for frame in self.frames if frame.addr in evo_region_addrs
        ]
    ...
    def set_multi_bits(
        self, bit_seq: Sequence[XC7BitPosition], value_seq: Sequence[bool]
    ) -> None:
    ...
    def get_evo_frame_bits(self) -> List[XC7BitPosition]:
        ...
        """
        return [bit for frame in self.evo_frames for bit in frame.to_bit_list()]
    ...
    def from_bytes(cls, bs_bytes: bytes, part_json: Union[Path, str]) -> "XC7BSHandler":
        ...
        with open(part_json) as f:
            ...
            order_packets(
                cfg_packets, init_packets, temp_frames, suffix_packets, addr_gen
            )
            frames = list()
    ...
    def habitat_bs_bytes(self) -> bytes:
        ...
        payload = reduce(
            ...
            [
                packet.bytes
                for packet in self.full_init_packets
                + main_payload
    ...
    def partial_evo_bytes(self) -> bytes:
        ...
        payload = reduce(
            ...
            [
                packet.bytes
                for packet in self.partial_init_packets
                + main_payload
...
def grab_idcode_from_packets(cfg_packets: List[ConfigPacket]) -> bytes:
    packet = [
        p
        for p in cfg_packets
        if p.config_word
        == ConfigWord(PacketType.TYPE1, OPCode.WRITE, 1, register=Register.IDCODE)
    ][0]
...
def read_init_packets(cfg_packets: list, init_packets: list) -> List[ConfigPacket]:
    for idx, packet in enumerate(cfg_packets):
        if (
            packet.config_word.register == Register.CMD
            and packet.payload == b"\x00\x00\x00\x01"
        ) or packet.config_word.register == Register.FAR:
...
def order_packets(
    cfg_packets: List[ConfigPacket],
    init_packets: list,
    frames: list,
    suffix_packets: list,
    addr_gen: FrameAddressGenerator,
) -> None:
    """
...
def read_frame_data(
    cfg_packets: List[ConfigPacket], frames: list, addr_gen: FrameAddressGenerator
) -> List[ConfigPacket]:
    while True:
        if (
            cfg_packets[1].config_word.op_code
            == OPCode.NOP
            == cfg_packets[3].config_word.op_code
            ...
            cfg_packets[2].config_word.op_code == OPCode.NOP
            and cfg_packets[0].config_word.register == Register.FAR
            and cfg_packets[3].config_word.register == Register.FDRI
        ):
        ...
        if type2_write:
            ...
            offset = 1
        if cfg_packets[offset].config_word == ConfigWord(
            PacketType.TYPE1, OPCode.WRITE, 1, Register.CMD
        ) and cfg_packets[offset].payload == bytes.fromhex("00000007"):
...
def next_chunk_is_mfw(cfg_packets: List[ConfigPacket], type2_write: bool) -> bool:
    ...
    # Check COMPRESSED
    if (
        cfg_packets[cmd_idx].config_word.register == Register.CMD
        ...
        (
            cfg_packets[cmd_idx].config_word.register == Register.CMD
            and cfg_packets[cmd_idx].payload == b"\x00\x00\x00\x01"
        )
...
def first_write_is_type2(cfg_packets: List[ConfigPacket]) -> bool:
    if (
        cfg_packets[0].config_word
        == ConfigWord(PacketType.TYPE1, OPCode.WRITE, 0, Register.FDRI)
        and cfg_packets[1].config_word.packet_type == PacketType.TYPE2
    ):
        return True
    elif cfg_packets[1].config_word == ConfigWord(
        PacketType.TYPE1, OPCode.WRITE, 1, Register.CMD
    ):
        return False
    else:
...
def read_mfw_chunk(
    ...
    if cfg_packets[16 + offset].config_word.op_code == OPCode.NOP:
        ...
        cfg_packets[16 + offset].config_word.register == Register.FAR
        or cfg_packets[16 + offset].config_word.register == Register.CMD
    ):
        nop_spacing = 2
        start = 14 + offset
    else:
    ...
    for idx in range(start, len(cfg_packets), nop_spacing):
        if cfg_packets[idx].config_word.register == Register.FAR:
            frames.append(
                Frame.from_bytes(
                    int.from_bytes(cfg_packets[idx].payload, "big"), frame_data
                )
            )
        else:
            return cfg_packets[idx:]
    ...
    def __next__(self):
        if self.idx < self.max_idx:
            ...
            if 4 * config_word.count > self.max_idx:
                raise Exception(
                    f"Error in bitstream format. ConfigWord declares {config_word.count} data words, "
