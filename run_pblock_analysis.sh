...
#    if [ ! -d "${output_path}/${pblock}/${ram_block}/bs" ]; then
    if [ -f "${output_path}/${pblock}/${ram_block}/bs/${ram_block}_00.bit" ] && [ -f "${output_path}/${pblock}/${ram_block}/bs/${ram_block}_ff.bit" ] && [ -f "${output_path}/${pblock}/${ram_block}/bs/${ram_block}_bramless_partial.bit" ] && [ -f "${output_path}/${pblock}/${ram_block}/bs/${ram_block}_partial_bram_bs.bit" ] && [ -f "${output_path}/${pblock}/${ram_block}/bs/${ram_block}_modified_partial.bin" ]; then
        ...
        row_min_y=$((($current_bram_y_position/12 ) *12 ))
        row_max_y=$((($current_bram_y_position/12 + 1)*12-1))

        "${vivado_path}" -mode batch -source tcl_scripts/synthesize_for_bram_block_x.tcl -tclargs "$project_xpr" "$pblock" "$bram_row_x_position" "$row_min_y" "$row_max_y" "$current_bram_y_position" > "${output_path}/${pblock}/${ram_block}/vivado.log"
    ...
    flash_bitstreams "${full_bs_with_initial_value_00_local}" "${bramless_partial_bs_local}" "${modified_bs_local}" "${wait_time}" "true";
