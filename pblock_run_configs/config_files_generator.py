...
argparser = argparse.ArgumentParser(
    "Creates config.sh files for 'run_pblock_analysis.sh'"
)
...
argparser.add_argument(
    "-v", "--vivado_path", help="Path to vivado executable", required=True
)
argparser.add_argument(
    "-ep",
...
argparser.add_argument(
    ...
    help="Interface used for flashing the bitstream "
    "(used internally by vivado)",
    required=True,
)
argparser.add_argument(
    "-b", "--board", help="Name of the board used", required=True
)
argparser.add_argument(
    "-f", "--fpga", help="Name of the used fpga device", required=True
)
argparser.add_argument(
    "-w",
    "--wait_time",
    help="Time that shall be waited between deactivating "
    "and reactivating brams",
    default=0,
    required=False,
    type=int,
)
argparser.add_argument(
    ...
    help="If used, bitstreams and reads with previous "
    "value ff will be performed",
    action="store_true",
)
argparser.add_argument(
    "-pmin",
...
if __name__ == "__main__":
    ...
    for pblock_idx in range(args.pblock_first_idx, args.pblock_last_idx + 1):
        ...
        write_config_file(
            output_path=Path(
                args.output_path, f"{args.board}_pblock_{pblock_idx}.sh"
            ),
        ...
        if current_bram_column == args.x_index - args.pblock_per_row + 1:
            ...
            if current_row > args.y_index + args.row_count:
                raise Exception(
                    "Reached last row before iterating through all pblocks. "
