...
def normalize_frequency_lists(
    zero_count_list: List[int], one_count_list: List[int]
...
def contingency_table_list(
    read_sessions: Dict[str, ReadSession], bit_type: str, normalize: bool
    ...
    for byte_idx in range(
        ...
        for bit_idx in range(8):
            ...
            # TODO Review this solution to observed 0 frequency
            if normalize:
                zero_count_list, one_count_list = normalize_frequency_lists(
                    zero_count_list, one_count_list
                )

            contingency_table_list.append(
                ContingencyTable(
                    variables=variables,
...
def add_bit_type_group(
    parent: h5py.Group, read_sessions: Dict[str, ReadSession], bit_type: str
) -> None:
    contingency_tables = contingency_table_list(
        read_sessions, bit_type, normalize=False
    )
    normalized_contingency_tables = contingency_table_list(
        read_sessions, bit_type, normalize=True
    )
    ...
    bit_group.attrs["Percentage of bits with pvalue > alpha:=0.05"] = (
        chi2_pvalue_percentage_pass(0.05, bit_chi2_results)
    )
    bit_group.attrs["Percentage of bits with pvalue > alpha:=0.1"] = (
        chi2_pvalue_percentage_pass(0.1, bit_chi2_results)
    )
...
def main():
    ...
    bram_block = list(
        list(list(experiment.boards.values())[0].pblocks.values())[
            0
        ].bram_blocks.values()
    )[
        0
